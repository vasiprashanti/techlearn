...
export const serializeAssignment = async (assignment, { includeQuestionContent = true } = {}) => {
  ...
  return {
    ...
    answeredQuestions: assignment.answeredQuestions,
...
export const getOrCreateProgramAssignment = async ({
  ...
  // A completed assignment is the learner's single recorded attempt. It can
  // be viewed again, but must never be regenerated by a refresh or retry.
  if (existing && (!force || existing.status === "Completed")) return existing;

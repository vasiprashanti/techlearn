...
import { FiBell, FiArrowLeft, FiUsers, FiActivity, FiTrendingUp, FiClock, FiBriefcase, FiCalendar, FiBookOpen } from 'react-icons/fi';
        ...
        <div className="max-w-[1600px] mx-auto space-y-8">
          ...
          <section className="space-y-8">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="bg-white/70 dark:bg-white/5 border border-black/5 dark:border-white/10 rounded-2xl p-5 space-y-3 hover:shadow-md transition-shadow">
                <p className="flex items-center gap-2 text-black/55 dark:text-white/55"><FiUsers className="w-4 h-4" /><span className="text-xs font-medium">Total Students</span></p>
                <p className="text-2xl font-bold text-black dark:text-white leading-none">{batch.students}</p>
              </div>
              <div className="bg-white/70 dark:bg-white/5 border border-black/5 dark:border-white/10 rounded-2xl p-5 space-y-3 hover:shadow-md transition-shadow">
                <p className="flex items-center gap-2 text-black/55 dark:text-white/55"><FiActivity className="w-4 h-4" /><span className="text-xs font-medium">Active Students Today</span></p>
                <p className="text-2xl font-bold text-black dark:text-white leading-none">{activeToday}</p>
              </div>
              <div className="bg-white/70 dark:bg-white/5 border border-black/5 dark:border-white/10 rounded-2xl p-5 space-y-3 hover:shadow-md transition-shadow">
                <p className="flex items-center gap-2 text-black/55 dark:text-white/55"><FiTrendingUp className="w-4 h-4" /><span className="text-xs font-medium">Average Score</span></p>
                <p className="text-2xl font-bold text-black dark:text-white leading-none">{batch.avgScore}%</p>
              </div>
              <div className="bg-white/70 dark:bg-white/5 border border-black/5 dark:border-white/10 rounded-2xl p-5 space-y-3 hover:shadow-md transition-shadow">
                <p className="flex items-center gap-2 text-black/55 dark:text-white/55"><FiClock className="w-4 h-4" /><span className="text-xs font-medium">Average Streak</span></p>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {batch.tracks.map((track) => (
                  <div key={track.name} className="bg-white/70 dark:bg-white/5 border border-black/5 dark:border-white/10 rounded-2xl p-5 space-y-3">
                    <h4 className="text-base font-bold text-black/85 dark:text-white inline-flex items-center gap-2"><FiBookOpen className="w-4 h-4 text-[#3C83F6] dark:text-blue-300" />{track.name}</h4>
                    <p className="text-xs text-black/45 dark:text-white/45">{track.questionsAssigned} questions assigned</p>
                    <div className="space-y-1.5 max-h-40 overflow-y-auto pr-1">
                      {track.days.length === 0 ? (
                        <p className="text-sm text-black/45 dark:text-white/45">No day assignments yet.</p>
                      ) : (
                        track.days.map((dayItem, index) => (
                          <div key={`${track.name}-${index}`} className="flex items-center gap-2 text-xs">
                            <span className="font-semibold text-[#3C83F6] dark:text-blue-300 w-10 shrink-0">Day {index + 1}</span>
                            <span className="text-black/80 dark:text-white/85 truncate">{dayItem}</span>
                          </div>
                        ))
                      )}
                    </div>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-black/5 dark:border-white/10">
                      <th className="text-left text-xs font-semibold text-black/45 dark:text-white/45 px-6 py-4">Student Name</th>
                      <th className="text-left text-xs font-semibold text-black/45 dark:text-white/45 px-6 py-4">Email</th>
                      <th className="text-left text-xs font-semibold text-black/45 dark:text-white/45 px-6 py-4">Score</th>
                      <th className="text-left text-xs font-semibold text-black/45 dark:text-white/45 px-6 py-4">Streak</th>
                    </tr>
                  </thead>
                  <tbody>
                    {batch.studentsTable.map((student, index) => (
                      <tr key={`${student.email}-${index}`} className="border-b border-black/5 dark:border-white/10 last:border-b-0 hover:bg-black/[0.02] dark:hover:bg-white/[0.03] transition-colors">
                        <td className="px-6 py-4 text-sm font-medium text-black/85 dark:text-white">{student.name}</td>
                        <td className="px-6 py-4 text-sm text-black/55 dark:text-white/60">{student.email}</td>
                        <td className="px-6 py-4">
                          <span className="inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold border-emerald-500/25 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400">{student.score}%</span>
                        </td>
                        <td className="px-6 py-4 text-sm text-black/60 dark:text-white/60"><span className="font-medium text-black/85 dark:text-white">{String(student.streak).split('/')[0].trim()}</span> / {String(student.streak).split('/')[1]?.trim() || '-'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>

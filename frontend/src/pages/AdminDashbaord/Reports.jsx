        ...
        <main className={`flex-1 transition-all duration-700 ease-in-out z-10 ${sidebarCollapsed ? 'lg:ml-20' : 'lg:ml-64'} pt-8 pb-12 px-6 md:px-12 lg:px-16 overflow-auto ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          <div className="max-w-[1400px] mx-auto space-y-8">
            ...
            <header className="flex items-center justify-between pb-6 border-b border-black/5 dark:border-white/5">
              <div>
                <h1 className="text-lg sm:text-xl md:text-2xl lg:text-4xl font-normal tracking-tight text-[#3C83F6] dark:text-white">Reports</h1>
                    ...
                    {report.formats.map(fmt => {
                      ...
                      return (
                        <button
                          ...
                          className={`flex items-center gap-1.5 text-[10px] uppercase tracking-widest px-3 py-1.5 rounded-lg border font-medium transition-all ${fs.bg} ${fs.text} hover:opacity-80 ${isLoading ? 'opacity-50 cursor-wait' : ''}`}
                        >
            ...
            {/* Recent Exports */}
            <div>
              <h2 className="text-xs tracking-widest uppercase text-black/50 dark:text-white/50 mb-4">Recent Exports</h2>

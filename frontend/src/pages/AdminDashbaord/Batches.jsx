  ...
  return (
    <article className={`relative rounded-2xl overflow-hidden border ${selected ? 'border-[#3C83F6] ring-1 ring-[#3C83F6]/50 dark:border-blue-400 dark:ring-blue-400/50' : 'border-black/10 dark:border-white/15'} bg-white/80 dark:bg-[#0f1f43] backdrop-blur-xl shadow-[0_3px_10px_rgba(15,23,42,0.04)] dark:shadow-[0_6px_16px_rgba(0,0,0,0.15)] h-full flex flex-col hover:bg-white dark:hover:bg-[#162a52] hover:shadow-md transition-all duration-300 group`}>
      ...
      <div className={`absolute right-3 top-3 z-20 batch-actions-${batch.id}`}>
        ...
        {menuOpen && (
          <div className="absolute right-0 top-8 w-36 rounded-xl border border-black/10 dark:border-white/15 bg-white/95 dark:bg-[#0f1f43] backdrop-blur-xl shadow-xl overflow-hidden z-20">
            <button
              onClick={() => {
                setMenuOpen(false);
                onEdit(batch);
      ...
      {/* pl-12 to account for checkbox on the left */}
      <div className={`px-4 pt-4 pb-3.5 min-h-[76px] border-b border-black/10 dark:border-white/15 ${theme.topTint} pl-12 pr-10 flex items-center`}>
        <div className="flex items-center justify-between gap-2.5 text-left w-full">
          <div className="flex-1 min-w-0">
            <h3 className="text-sm md:text-[15px] leading-snug font-bold text-slate-900 dark:text-white truncate">{batch.name}</h3>
      ...
      <div className="px-4 py-3.5 mt-auto bg-white/70 dark:bg-transparent flex flex-col gap-2 text-left">
        <div className="flex items-center justify-between gap-3 text-[11px] md:text-xs text-slate-550 dark:text-slate-400">
          <span>Active Track</span>
          <span className="font-semibold text-slate-800 dark:text-slate-200 truncate max-w-[130px]">{batch.track || 'No Track'}</span>
        </div>
        <div className="flex items-center justify-between gap-3 text-[11px] md:text-xs text-slate-550 dark:text-slate-400">
          <span>Students</span>
        ...
        <button
          ...
          className="mt-2.5 w-full h-9 rounded-xl bg-[#3C83F6] hover:bg-[#2f73e0] dark:bg-[#bceaff] dark:hover:bg-[#a6e2ff] dark:text-[#06224d] text-white text-[12px] font-semibold transition-colors flex items-center justify-center gap-1.5"
        >
          View Batch
        </button>
      </div>

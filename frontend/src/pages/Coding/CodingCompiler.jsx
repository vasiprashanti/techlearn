  ...
  const CodingCompiler = () => {
    ...
    const [loadingProblems, setLoadingProblems] = useState(true); // NEW
      ...
      const fetchProblems = async () => {
        try {
          ...
          if (Array.isArray(data) && data.length > 0) {
            ...
            console.error("No problems found from API");
          }
        } catch (err) {
          console.error("Error fetching problems", err);
        } finally {
          setLoadingProblems(false); // always stop loading
        }
      };
      fetchProblems();
    }, []);
    ...
    // post results to backend and show summary
    ...
    if (!PROBLEM) {
      return (
        <div className="flex items-center justify-center h-screen">
          <p className="text-lg text-gray-600 dark:text-gray-300">
            ❌ No problems available. Please check your API.
          </p>
        </div>
      );
    }
